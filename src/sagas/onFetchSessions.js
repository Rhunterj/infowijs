...
const onFetchSessions = function* onFetchSessions() {
  yield takeLatest(
    actionTypes.SESSIONS.FETCH_SESSIONS,
    function* onfetchSessions({ meta: { page, limit = 20, filter = false } }) {
      if (filter) {
        yield put(
          setSessions(
            [...data]
              .filter(
                (session) =>
                  session.customer.name.toLocaleLowerCase().includes(filter.toLocaleLowerCase()) ||
                  session.user.id.toString().includes(filter.toString()) ||
                  session.user.name.toLocaleLowerCase().includes(filter.toLocaleLowerCase())
              )
        ...
        yield put(
          setSessions([...data].splice((page - 1) * limit, limit), {
            total: data.length,
            limit,
            page: Number(page),
          })
